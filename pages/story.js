  ...
  render() {
    ...
    return (
      <Main>
        ...
        <List>
          { story.comments && story.comments.map(item => {
            return (
                                    <List.Item>
                      <Image avatar src={`https://ui-avatars.com/api/?name=${item.user}`}  />
                      <Head as='h5'>{item.user}</Head>
                      <List.Content>
                        <List.Header as='em'>Posted {item.time_ago}</List.Header>
                        <List.Description dangerouslySetInnerHTML={{__html: item.content}}>
                        </List.Description>
                      </List.Content>
                    </List.Item>
            )
